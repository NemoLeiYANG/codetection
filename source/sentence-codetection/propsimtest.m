function [boxes_w_fscore,gscore,num_gscore]= ... %d_score,w_score,s_score,G,distance_threshold,worldDist,phists,valid_loc]  
    propsimtest(top_k, ssize, frames, positions,alpha,beta,gamma)
...
% compute
[h, w, d, T] = size(frames);
bboxes = zeros(top_k, 9, T); %each row: [x y w h score xloc yloc wwidth_pdist wwidth_norm]
