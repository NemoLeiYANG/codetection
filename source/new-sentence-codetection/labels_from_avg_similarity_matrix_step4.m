function labels = labels_from_avg_similarity_matrix_step4(avg_similarity_matrix)
...
%                         end %if
                    ...
                    if (length(label_idx) == 1)
                        %                             fprintf('Success 2\n');
                        if (label_idx > length(other_labels))
                            fprintf('Waiting for next iteration\n');
                            display(other_labels)
                            display(label_idx)
                            display(ratio_vals)
                            display(i)
...
%     relaxed_val_ratio_threshold = 0.95;%1 * val_ratio_threshold;
