function [numpeaks, peakxys,scores_out] = find_objects_new(detection_data, xmin,...
...
% sizethresh = 15000;
% if (N > sizethresh) %need to downsample
%     idx = randi([1 N], (N-sizethresh),1);
%     idx = sort(idx,'descend');
%     for i = 1:length(idx)
%         detection_data(idx(i),:) = [];
%     end %for i
% end %if N

% bad_points = [];
% %filter out detections that are way out of bounds
% for i = 1:N
%     if (detection_data(i,1) > (xmax + 0.5))
%         bad_points = [bad_points, i];
%         continue;
%     end %if
%     if (detection_data(i,1) > (xmin - 0.5))
%         bad_points = [bad_points, i];
%         continue;
%     end %if
%     if (detection_data(i,2) > (ymax + 0.5))
%         bad_points = [bad_points, i];        
%         continue;
%     end %if
%     if (detection_data(i,2) > (ymin - 0.5))
%         bad_points = [bad_points, i];
%         continue;
%     end %if 
% end %for N
% bad_points = sort(bad_points,'descend');
% for i = 1:length(bad_points)
%     detection_data(bad_points(i),:) = [];
%     [len,~] = size(detection_data);
%     if (len < 1000)
%         break;
%     end
% end %for i
...
[xx yy] = meshgrid(detection_data(:,1),detection_data(:,2)); %precomputed
prop_scores = detection_data(:,3);
freqs = detection_data(:,4);
for i = 1:num_ypoints
    for j = 1:num_xpoints
        %fprintf('starting j = %d\n',j);
        scorept = [xpoints(j),ypoints(i)];
        d = diag(sqrt((xx-scorept(1)).^2+(yy-scorept(2)).^2));
        dscores = gaussmf(d,[gaussian_variance,0]);
        allscores = dscores .* prop_scores .* freqs;
        %allscores = log10(dscores+eps) + log10(prop_scores) + log10(freqs);
        scores(i,j) = sum(allscores);
%         for n = 1:N
%             scorept = [xpoints(j),ypoints(i)];
