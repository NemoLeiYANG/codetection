...
double bp_sentence_codetection_inference_simple(int num_nouns, int num_tubes,
  ...
  mytime = time(NULL);
  printf("--------------------\n");
  printf("Finshed bp_sentence_codetection_inference_simple at ");
  printf(ctime(&mytime));
  return bp.value();
}
...
extern "C" double run_inference_1and2(double **f, double **g, 
					  int T, int top_k, 
  ...
  time_t mytime;
  mytime = time(NULL);
  printf("\nStarting run_inference_1and2 at ");
  printf(ctime(&mytime));
  printf("--------------------\n");
  ...
  for (unsigned int t = 0; t < numVariables; t++){
    ...
    if (vars[t] > 1){
      //get nonzero unary score values
      unsigned int j = 0;
	...
	if (f[t][i] > 0){
      ...
      for (unsigned int i = 0; i < (vars[t] - 1); i++){
	ff(i) = temp_scores[i];
      }
    } //otherwise only dummy exists, nothing to do
    fid = gm.addFunction(ff);
    // add factors
    size_t fv[] = {size_t(t)};
    gm.addFactor(fid, fv, fv+1);
...
extern "C" double bp_object_inference_new(double **f, double **g, 
  ...
  printf("in bp_object_inference_new\n");
  //typedef opengm::external::libdai::Bp<GraphModel, opengm::Minimizer> libdaiBP;
